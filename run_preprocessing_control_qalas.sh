...
# Loop over all CONTROL subjects & do the preprocessing
...
#
# Here we use the T1 from QALAS (qMRI)
#
# Updated 11/02/2022



for i in /home/denbo78/test_folder_covid_preproc/c* ; do

    cd $i
    echo "Processing" $i
    
    # make a temporary directory for analysis
    mkdir analysis_qalas

    # copy the mdd_long & anatomical to this folder
    cp MDDlong/mdd_long.nii analysis_qalas
    cp *QALAS*/*e3*.nii analysis_qalas

    # copy the bvals & bvecs into the analysis directory
    cp -a /home/denbo78/Triumph_Git/mddLong_experimental_params/. analysis_qalas
    ...
    3dresample -dxyz 0.5 0.5 0.5 -input *e3*.nii -prefix anat_resampled.nii
